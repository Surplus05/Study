...
const a:PersonWithBirthDate = {firstName:'a', lastName:'b', birthDate:new Date()}
...
```ts
const c1:Duo<{last: string, age:number}> = [{last:'Kim',age:10},{last:"Kim", age: 10}]
```
...
그러면 A['first'] 로 접근되고, 당연히 오류가 발생한다.  
